...
if(!$stmt) {
    ...
    if($numrows == 0) {
        ...
        $query = "SELECT
                  ...
                  WHERE
                    topic_cat=?" . (isset($_SESSION['signed_in']) && $_SESSION['signed_in'] && $_SESSION['user_level'] == 1 ? '' : " AND topic_visible='TRUE'" ) . "
        ...
        if(!$stmt) {
            ...
            if($numrows == 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<tr class='$class'" . ($row['topic_visible'] ? '' : ' style="background-color: #ffa1a1"') . ">";
